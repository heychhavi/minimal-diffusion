...
class DiffusionBertForSequenceRegression(nn.Module):
    ...
    def loss_from_outputs(self, outputs, labels=None):
        ...
        regression_output = self.classification_head(pooled_output).squeeze(-1)  # Ensure the output shape is compatible with labels
    
        if labels is not None:
          # Ensure labels are float for regression
          labels = labels.float()
          loss_fct = nn.MSELoss()
          loss = loss_fct(regression_output, labels)
        else:
          loss = None

        return SequenceClassifierOutput(
            loss=loss,
            logits=regression_output,  # Here, logits would actually be the regression outputs
    ...
    def load_from_checkpoint(
        ...
        model = DiffusionBertForSequenceRegression(config, diffusion_model, num_labels)
...
def train_classifier_on_diffusion_latents():
    ...
    model = DiffusionBertForSequenceRegression(
